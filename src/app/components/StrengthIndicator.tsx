...
export default function StrengthIndicator({ strength }: StrengthIndicatorProps) {
    const getStrengthColor = () => {
        switch (strength) {
            case 'TOO WEAK!': return 'bg-[#F64A4A]';
            case 'WEAK': return 'bg-[#FB7C58]';
            case 'MEDIUM': return 'bg-[#F8CD65]';
            case 'STRONG': return 'bg-[#A4FFAF]';
            default: return 'bg-[#817D92]';
        }
    };
            ...
            <div className="flex items-center space-x-4">
                ...
                <div className="flex space-x-1">
                    {[...Array(4)].map((_, index) => (
                        <div
                            key={index}
                            className={`w-2 h-6 border-2 ${index < getStrengthBars()
                                ? getStrengthColor()
